...
class FindingCandidate(BaseSchema):
    ...
    cvss_score: Optional[float] = Field(
        ...
        ge=0.0,
        le=10.0,
        description="CVSS score - DEPRECATED: use cvss object instead",
    )
    ...
    def get_cvss_object(self) -> Optional[CVSS]:
        """
        Get CVSS object, creating from legacy field if needed.
        ...
        if self.cvss_score is not None:
            return CVSS.from_score(score=self.cvss_score)
    ...
    def set_cvss(
        ...
        status: CVSSAssessmentStatus = CVSSAssessmentStatus.CONFIRMED,
    ...
    def get_cvss_display(self) -> str:
        """Get formatted CVSS display string for reports."""
        if self.cvss:
    ...
    def get_cvss_for_report(self) -> dict:
        """Get CVSS data formatted for report output."""
        if self.cvss:
            return {
                "version": self.cvss.cvss.version.value,
                "vector": self.cvss.cvss.vector,
                "base_score": self.cvss.cvss.base_score,
                "severity": self.cvss.cvss.severity.value if self.cvss.cvss.severity else "unknown",
                "status": self.cvss.status.value,
                "source": self.cvss.source,
            }
        elif self.cvss_score is not None:
            severity = CVSS._score_to_severity(self.cvss_score)
            return {
