...
Note: This passer handles the GitHub MCP which is separate from hexstrike-ai.
"""
...
class GitHubPasser(BasePasser):
    ...
    Note: GitHub is a separate MCP from hexstrike-ai.
    """

    category = HexstrikeToolCategory.GITHUB
    supported_tools: List[HexstrikeTool] = []  # GitHub MCP uses different tool names
    tool_name = "github"  # For observation creation
    ...
    def normalize(
        ...
        tool: HexstrikeTool = HexstrikeTool.UNKNOWN,
        **kwargs: Any,
    ) -> PasserResult:
        """
        ...
        Args:
            ...
            tool: Tool type (not used for GitHub MCP).
