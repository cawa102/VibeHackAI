...
class TestAuditEvent:
    ...
    def test_event_with_details(self):
        ...
        event = AuditEvent(
            ...
            session_id="session-001",
            timestamp=datetime.utcnow(),
            details={"context_size": 1024},
            agent="recon_agent",
            phase="recon",
        )

        assert event.details["context_size"] == 1024
        assert event.agent == "recon_agent"
    ...
    def test_event_to_dict(self):
        ...
        event = AuditEvent(
            ...
            details={"from_phase": "recon", "to_phase": "enumeration"},
            phase="enumeration",
        )
        ...
        assert result["details"]["from_phase"] == "recon"
        assert result["details"]["to_phase"] == "enumeration"
...
class TestOrchestratorAuditLogger:
    ...
    def test_log_agent_invoked(self, logger, mock_state_store):
        ...
        assert event_data["agent"] == "recon_agent"
    ...
    def test_log_approval_requested(self, logger, mock_state_store):
        ...
        assert event_data["approval_id"] == "req-001"
    ...
    def test_log_patch_received(self, logger, mock_state_store):
        ...
        assert event_data["patch_id"] == "patch-001"
        assert event_data["details"]["operations_count"] == 5
    ...
    def test_log_patch_rejected(self, logger, mock_state_store):
        ...
        assert event_data["details"]["reason"] == "Validation failed"
        assert len(event_data["details"]["errors"]) == 2
    ...
    def test_log_error(self, logger, mock_state_store):
        ...
        assert event_data["details"]["error"] == "Something went wrong"

    def test_count_events(self, logger, mock_state_store):
        """Test counting events."""
        # Setup mock to return events from read_jsonl
        mock_state_store.read_jsonl.return_value = [
            {"session_id": "test-session", "event_type": "session_start"},
            {"session_id": "test-session", "event_type": "phase_transition"},
            {"session_id": "test-session", "event_type": "agent_invoked"},
        ]

        count = logger.count_events()
        assert count == 3

    def test_timestamps_unique(self, logger, mock_state_store):
        """Test timestamps are recorded for each event."""
        timestamps = []
...
class TestAuditLoggerQuery:
    ...
    def test_get_events_by_type(self, mock_state_store):
        ...
        # events is a list of AuditEvent objects, not dicts
        assert all(e.event_type == AuditEventType.PHASE_TRANSITION for e in events)
